"""
Graph rendering — pure QPainter-on-QImage rendering logic.

Extracted from export_dialog.py so the dialog is just UI and the
rendering pipeline can be reused (e.g. for batch export, headless tests,
or copy-to-clipboard). Kept in ``gui/`` because it depends on Qt; it is
not part of ``processing/`` (which is Qt-free).
"""
